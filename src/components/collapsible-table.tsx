...
export function CollapsibleDataTable<TData>({
    ...
    const table = useReactTable({
        ...
        manualPagination: !!paginationProps?.totalPages || !!cursorPagination || viewAll,
                        ...
                        {table.getHeaderGroups().map(headerGroup => (
                            <TableRow key={headerGroup.id}>
                                {headerGroup.headers.map((header, i) => (
                                    <TableHead
                                        key={i}
                                        className={cn(rowSelection && header.index === 0 && "w-8", 'text-xs px-4')}
                                    >
                        ...
                        {table.getRowModel().rows.length > 0
                            ? table.getRowModel().rows.map((row, i) => (
                                <TableRow
                                    ...
                                    className={cn(`hover:bg-border/50 border-none ${rowColor?.(row.original)}`, (!row.original?.id || row.getCanExpand()) ? 'bg-muted' : '', row.getIsExpanded() && 'bg-border/40', 'h-full')}
            ...
            {!viewAll && (
                <div className="pt-4 mx-auto w-max">
                    {paginationProps?.totalPages ? (
                        <ParamPagination disabled={disabled || loading} {...paginationProps} />
                    ) : cursorPagination ? (
                        <CursorPagination {...cursorPagination} disabled={disabled || loading} />
                    ) : (
                        <ParamPagination
